    ...
    public static function funccallSource($stackClass, $context, $data, $myClass, $instruction)
    {
        ...
        if (!is_null($mySource)) {
            ...
            if ($exprReturn->isAssign()) {
                 ...
                 */
                // Reset Taint
                // $arrParameter = $mySource->getParameters();
                // foreach ($arrParameter as $parameter) {
                //     $nbParams = $parameter - 1;
                //     $defArg = $instruction->getProperty("argdef$nbParams");
                //     if (!is_null($defArg)) {
                //         $condition_value = $mySource->getconditionsParameter($parameter, MySource::CONDITION_VALUE);
                //         if (!is_null($condition_value)) {
                //             // var_dump(($defArg->getLastKnownValues()));
                //             if (!empty($defArg->getLastKnownValues())) {
                //                 $myDef->setTainted(false);
                //             }
                //         }
                //     }
                // }
